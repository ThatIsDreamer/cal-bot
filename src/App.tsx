...
function App() {
  ...
  const [program, setProgram] = useState<"comfortable" | "fast" | "slow">(
    "comfortable"
  ); // новое состояние
  const totalSteps = 8; // увеличено на 1
  const [editingFromSummary, setEditingFromSummary] = useState<number | null>(
    null
  ); // если не null, значит редактируем из финального шага
  ...
  useEffect(() => {
    ...
    if (backButton.onClick.isAvailable()) {
      unsubscribe = backButton.onClick(handleBack);
    }

    // Show/hide back button based on step
    if (step > 0 && backButton.show.isAvailable()) {
      backButton.show();
    } else if (step === 0 && backButton.hide.isAvailable()) {
      backButton.hide();
    }
        ...
        {step === 0 && (
          ...
          >
            <img
              ...
              height={180}
              src={myAnimation}
              alt="A looping animation"
            />
            <h1>Привет!</h1>
            <p>CalBot - это твой путь к здоровью!</p>
        ...
        {step === 6 && (
          ...
          >
            ...
            <Tabs
              ...
              value={program}
              onValueChange={(val) =>
                setProgram(val as "comfortable" | "fast" | "slow")
              }
            >
              <TabsList>
          ...
          >
            ...
            >
              ...
              >
                ...
                />
                <span style={{ fontSize: 18 }}>
                  Желаемый вес: <b>{desiredWeight} кг</b>
                </span>
                <ArrowRight size={20} />
              </div>
              <div
